...
(function (XY) {
  ...
  function renderPhoto(opts) {
    ...
    var svg = XY.svgEl('svg', {
      ...
      /* The top is -210, not -56: the tricorn now sits above the cropped head
         and reaches roughly y=-120 with its peaks. `overflow: visible` would
         still paint it, but a viewBox that actually contains its content keeps
         the actor's own box honest. */
      viewBox: '-96 -210 764 1010', class: 'creature hero hero-photo',
      'aria-hidden': 'true', focusable: 'false', overflow: 'visible'
    });
    var root = XY.svgEl('g', { class: 'c-root' });
    var body = XY.svgEl('g', { class: 'c-body' });
    ...
    if (withHat && XY.Creature && XY.Creature.shapes) {
       ...
       * ----------------------------------------------------------------*/
      /* ------------------------------------------------------------------
       *  The hat SITS ON the cut, it does not come down over it.
       *
       *  Her head carries on above the top of the frame, so the hat belongs in
       *  that missing space — resting on the crop line, not pulled down onto
       *  her face. Getting that right is fiddlier than it sounds, because the
       *  tricorn's lower edge is a curve, not a straight line: measured from
       *  its centre it hangs +17 in the middle, +15 at 0.45w, +9 at 0.9w, and
       *  the turned-up peaks at ±w ride 10 ABOVE centre.
       *
       *  So "rest it on the line" has no single answer — the brim meets a
       *  horizontal at a different height depending where you measure. Solving
       *  at the centre leaves the ends floating and the flat cut shows under
       *  them. Solving at the peaks (the first attempt) buries her forehead.
       *
       *  Solve it where the CUT ENDS, since that is the only span that has to
       *  be met: half the cut is ~150px, which at this scale lands around
       *  0.66w along the brim, where the lower edge sits about +12.
       * ----------------------------------------------------------------*/
      var HAT_OVERHANG = 1.5;                  // brim vs the width of the cut
      var BRIM_AT_CUT = 12;                    // lower edge at ~0.66w, in authored units
      var brimW = PH.cropW * HAT_OVERHANG;
      var kx = brimW / 116;
      /* Only mildly foreshortened now. The heavy squash existed because the hat
         was competing for forehead room; sitting above the crop it has none of
         that constraint and can look like a proper hat again. */
      var ky = kx * 0.85;
      var hw = brimW / 2;
